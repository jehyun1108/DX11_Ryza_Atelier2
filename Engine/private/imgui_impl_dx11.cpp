...
#endif // dear imgui: Renderer Backend for DirectX11
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the pixel shader
    {
        static const char* pixelShader =
            ...
            "{                                                        \n"
            "    // 1) 버텍스 컬러를 Linear로 변환 (알파는 그대로)          \n"
            "    float4 col = input.col;                              \n"
            "    col.rgb = SRGBToLinear(col.rgb);                     \n"
            "                                                         \n"
            "    // 2) 폰트/아이콘 텍스처는 UNORM(Linear)로 샘플           \n"
            "    float4 tex = texture0.Sample(sampler0, input.uv);    \n"
            "                                                         \n"
            "    // 3) Linear 공간에서 곱 → sRGB 백버퍼가 자동 인코딩       \n"
            "    return col * tex;                                    \n"
            "}                                                        \n";
...
#ifndef IMGUI_IMPL_DX11_BACKBUFFER_FORMAT
#define IMGUI_IMPL_DX11_BACKBUFFER_FORMAT DXGI_FORMAT_R8G8B8A8_UNORM_SRGB
#endif
