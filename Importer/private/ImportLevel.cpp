...
HRESULT ImportLevel::Init()
{
	...
	game.AddPanel<EffectPanel>("EffectTool");
#endif
	...
	//string errorMsg;
	//registry.Get<WorldSerializer>().LoadWorldFromFile(L"../bin/Resources/Map/map.dat", entities, errorMsg);
	filesystem::path tmp{ L"../bin/Resources/Map/navmesh.nav" };
	navSys->Load(tmp);
...
// --------------------------------------------------------------------------------------------------
	...
	auto cam = spawner->NewEntity()
		.WithTf(TransformDesc{ .pos = _float3{ 0.f, 150.f, 800.f } })
		.WithLayer(LayerUtil::LayerBit(LAYER::CAMERA))
		.WithCam(XMConvertToRadians(75.f), float(WinX) / WinY, 0.1f, 10000.f, true)
		.WithFreeCam(500.f)
...
EntityHandles ImportLevel::SpawnPatricia()
{
	auto patricia = spawner->NewEntity()
		...
		.WithColliderFromModel(ColliderType::OBB, Bit(CollisionLayer::Prop), Bit(CollisionLayer::Character) | Bit(CollisionLayer::Ground), true)
		.WithTag("patricia")
		.WithPlayerMovement()
		.Build();
...
void ImportLevel::UpdateGrid()
{
	...
	ctx.snap.enabled = input->KeyPressing(KEY::LSHIFT);
